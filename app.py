...
def flag_color(cls, p, c, h):
    if cls == "N":
        return "Safe" if p < 80 else "Elevated" if p < 90 else "Dangerous"
    if cls in ("A", "a", "S", "j", "J"):
    ...
    if cls == "L": return "Dangerous" if p>50 else "Safe"
    if cls == "R": return "Elevated" if p>50 else "Safe"
    if cls == "/": return "Elevated" if p>90 else "Safe"
    if cls == "E": return "Elevated" if c>=30 else "Safe"
    if cls == "F": return "Elevated" if c>10 else "Safe"
    if cls == "Q": return "Dangerous" if p>20 else "Elevated" if p>10 else "Safe"
    return "Safe"
...
def load_record(record_path, win=WINDOW):
    try:
        ...
        for pos, sym in zip(ann.sample, ann.symbol):
            ...
            seg = ecg[start:end]
            if len(seg)<win: seg = np.pad(seg,(0,win-len(seg)),'constant')
            beats.append(seg)
            labels.append(cls)
...
if dat_file and atr_file:
    ...
    if record_name != atr_file.name.split(".")[0]:
        ...
        if len(X_raw)==0:
            st.error("Failed to load ECG for ML.")
        else:
            ...
            shap_values_xgb=explainer_xgb.shap_values(X_te[:50])
        ...
        try:
            ...
            if os.path.exists(f"{record_name}.hea"): os.remove(f"{record_name}.hea")
        except: pass
